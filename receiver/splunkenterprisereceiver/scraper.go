...
// number of metric scrapes
const metricScrapes = 21
...
func (s *splunkScraper) shutdown(ctx context.Context) (err error) {
	close(s.deploymentInfo)
	close(s.errsOutChan)
	close(s.errsChan)
	return nil
}
...
func (s *splunkScraper) scrapeLicenseUsageByIndex(ctx context.Context, now pcommon.Timestamp) {
	...
	for {
		...
		if time.Since(start) > s.conf.ControllerConfig.Timeout {
			s.errsChan <- errMaxSearchWaitTimeExceeded
			return
		}
	}
